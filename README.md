...
| 🎴 카드팩 · 🗂️ 카드 앨범 | 포켓몬을 잡을 때마다 **카드팩** 1개 (전설·신화·시크릿·이로치는 ✨ **빛나는 카드팩**). 팩을 톡톡톡 세 번 두드리면 찢어지고, 포켓몬 카드 6,785장 중 한 장이 뒤집혀 나와요. 등급: ⚪ 일반 · 🔷 레어 · 🎨 아트 레어 · 💎 슈퍼 레어 · 👑 스페셜. 모은 카드는 앨범에서 크게 볼 수 있어요 |
...
카드 목록은 `data/pokemon_cards.xlsx`(카드명·종류·세트·희귀도·이미지 주소)로 만들어요. 목록을 바꾸면 `python3 tools/build-cards.py`를 실행하면 `js/cards.js`가 새로 만들어져요. 카드 그림은 저장소에 넣지 않고 포켓몬카드 공식 사이트의 이미지 주소로 연결해요. 그림을 못 불러오는 곳에서는 카드 이름이 적힌 카드로 보여 줘요. 카드 확률은 `js/data.js`의 `PACK_ODDS`에서 바꿀 수 있어요.
